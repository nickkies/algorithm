#[cfg(test)]
mod tests {
    use rand::Rng;
    use std::collections::HashSet;

    fn binary_search(arr: &[u32], target: u32) -> Option<usize> {
        let mut low = 0;
        let mut high = arr.len() - 1;

        while low < high {
            let mid = (low + high) / 2;
            let mid_value = arr[mid];

            if mid_value == target {
                return Some(mid);
            } else if mid_value < target {
                low = mid + 1;
            } else {
                high = mid;
            }
        }

        None
    }

    #[test]
    fn test_binary_search() {
        let sorted_array = vec![1, 2, 3, 4, 5, 7, 8, 9, 10];
        let target = 7;

        assert_eq!(binary_search(&sorted_array, target), Some(5));

        let target = 6;
        assert_eq!(binary_search(&sorted_array, target), None);
    }

    /// ### 문제
    ///
    /// N개의 정수 A[1], A[2], …, A[N]이 주어져 있을 때,
    /// 이 안에 X라는 정수가 존재하는지 알아내는 프로그램을 작성하시오.
    ///
    /// ### 입력
    ///
    /// 첫째 줄에 자연수 N(1 ≤ N ≤ 100,000)이 주어진다.
    /// 다음 줄에는 N개의 정수 A[1], A[2], …, A[N]이 주어진다.
    /// 다음 줄에는 M(1 ≤ M ≤ 100,000)이 주어진다.
    /// 다음 줄에는 M개의 수들이 주어지는데, 이 수들이 A안에 존재하는지 알아내면 된다.
    /// 모든 정수의 범위는 -2^31 보다 크거나 같고 2^31보다 작다.
    ///
    /// ### 출력
    ///
    /// M개의 줄에 답을 출력한다. 존재하면 1을, 존재하지 않으면 0을 출력한다.
    #[test]
    fn search_for_numbers() {
        let mut arr_n = get_array();
        let arr_m = get_array();

        let set_n = arr_n.iter().cloned().collect::<HashSet<i32>>();
        arr_n.sort();

        for m in arr_m {
            let result = search(&arr_n, m);

            assert_eq!(result, set_n.contains(&m));
        }
    }

    fn search(arr: &[i32], target: i32) -> bool {
        let mut low = 0;
        let mut high = arr.len() - 1;

        while low < high {
            let mid = (low + high) / 2;
            let mid_value = arr[mid];

            if mid_value == target {
                return true;
            } else if mid_value < target {
                low = mid + 1;
            } else {
                high = mid;
            }
        }

        false
    }

    fn get_array() -> Vec<i32> {
        let mut rng = rand::thread_rng();

        (1..=rng.gen_range(1..=100_000))
            .map(|_| rng.gen_range(i32::MIN..i32::MAX))
            .collect::<Vec<i32>>()
    }
}
